...
parser.add_argument('--test_file', type=str, default="/raid/home/joshua/MultiDCP/MultiDCP/data/pert_transcriptom/06212022_signature_test_cell_4_small.csv")
...
def predict():
    ...
    with torch.no_grad():
        ...
        output = test_epoch_end(epoch_loss = epoch_loss, lb_np = lb_np, 
                        predict_np = predict_np, steps_per_epoch = i+1, 
                        epoch = i, metrics_summary = metrics_summary,
                        job = 'perturbed', USE_WANDB = False)

    return render_template('index.html', prediction_text='Output of MultiDCP:{}'.format(output))
